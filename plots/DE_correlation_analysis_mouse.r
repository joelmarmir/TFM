...
# Load data
df <- read.csv("/users/genomics/jmartinez/data/07_for_results/fc_all_mouse_with_micro3_github.csv", header = TRUE)
...
############### Create a dendogram and reorder sibling branches  ######################
...
# Check dend
pdf("/users/genomics/jmartinez/dend_plot3.pdf")
plot(dend, main = "Column Dendrogram", horiz = FALSE)
dev.off()

# Swap branches: dend[[1]] <-> dend[[2]]
temp <- dend[[1]]
dend[[1]] <- dend[[2]]
dend[[2]] <- temp

# Convert back to hclust
hc_swapped <- as.hclust(dend)

# Use the same dendrogram for rows and columns (since cor_matrix is symmetric)
pdf("/users/genomics/jmartinez/heatmap72_v5_all_pearson_mouse.pdf", width = 10, height = 10)
pheatmap(
  cor_matrix,
  cluster_cols = hc_swapped,
  cluster_rows = hc_swapped,
